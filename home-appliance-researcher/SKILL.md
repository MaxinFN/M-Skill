...
> **强约束：品牌评级必须基于量化评分，禁止直接给"第一梯队/第二梯队"的定性结论。** 先打分，再定级。

## 5 维评分模型

每个品牌按以下 5 个维度独立打分（1-10 分），加权求和得出总分。

| 维度 | 权重 | 含义 | 数据来源 |
|------|:----:|------|----------|
| **核心部件自主度** | 30% | 品牌对品类核心部件的设计/制造控制深度。L5→10分，L4→8分，L3→5分，L2→3分，L1→1分 | 制造深度分级（Section #四）+ 拆机验证 |
| **整机品控一致性** | 25% | 同型号不同批次之间性能/用料的一致性。拆机对比同型号不同批次是否缩水 | 拆机横评、市场口碑、召回记录 |
| **售后覆盖密度** | 20% | 全国售后服务网点数量、响应速度、配件供应能力 | 品牌官网、315投诉数据、家电论坛 |
| **技术路线纯粹性** | 15% | 是否采用该品类公认的完整技术方案（如空调全直流+EEV+双排，不阉割关键部件） | 拆机验证 + 参数表分析 |
| **价格工程比** | 10% | 零售价 / BOM成本（估算）。比值越低说明品牌溢价越少，工程价值传递越直接 | BOM拆解（Section #三） |

**权重之和 = 100%**

## 评分量表

| 分数 | 核心部件自主度 | 品控一致性 | 售后覆盖 | 技术路线纯粹性 | 价格工程比 |
|:----:|---------------|:----------:|:--------:|:-------------:|:---------:|
| **9-10** | 核心部件自研自产（L5） | 从未出现批次缩水 | 全国县级覆盖+快速响应 | 全系标配完整技术方案 | 零售/BOM < 1.8 |
| **7-8** | 核心部件合资或自研（L4） | 偶有批次差异但不影响性能 | 地市级覆盖 | 主力型号完整，低端有阉割 | 1.8–2.2 |
| **5-6** | 整机自产但核心外购（L3） | 批次用料有波动 | 省会+重点城市覆盖 | 仅高端型号完整 | 2.2–2.8 |
| **3-4** | ODM规格设计（L2） | 批次一致性不可控 | 依赖第三方售后 | 规格由代工商主导 | 2.8–3.5 |
| **1-2** | 纯贴牌（L1） | 品质完全不可预测 | 售后基本无保障 | 无技术路线选择权 | > 3.5 |

## 梯队划分（按加权总分）

| 总分 | 梯队 | 标签 | 含义 |
|:----:|:----:|------|------|
| ≥ 8.0 | **第一梯队** | 技术主导型 | 核心部件自研+品控顶级+技术路线完整。长期使用价值最高 |
| 6.0–7.9 | **第二梯队** | 平衡型 | 性价比+稳定性平衡。是大多数用户的最优选择 |
| 4.0–5.9 | **第三梯队** | 成本/营销驱动型 | 功能堆叠或价格优先，长期可靠性或技术完整性有妥协 |
| < 4.0 | **不推荐** | 高风险 | 品质不可控，售后无保障，不应进入购买考虑 |

## 品类权重微调

> 核心部件自主度的品类定义见 Section #四。以下品类可微调权重：

| 品类 | 调整 | 原因 |
|------|------|------|
| 空调 | 核心部件自主度 +5%（30→35%），价格工程比 -5%（10→5%） | 压缩机自研 vs 外购对可靠性和TCO影响远超其他维度 |
| 电视 | 核心部件自主度 +5%（30→35%），价格工程比 -5% | 面板来源是电视产业竞争的第一性变量 |
| 净水器 | 售后覆盖 +5%（20→25%），技术路线 -5%（15→10%） | 滤芯配送+安装服务比技术路线的品牌差异更大 |
| 吸尘器/洗地机 | 品控一致性 +5%（25→30%），价格工程比 -5% | 高速电机的动平衡和电池一致性是制造硬功夫 |

> 未列出的品类使用默认权重。调整必须在报告中注明。

## 输出格式

- **品牌评分卡**：每个品牌列出 5 维得分 + 加权总分 + 梯队 + 一句话评语
- **至少覆盖 5 个品牌**，覆盖该品类的主流价格带
- **必须注明评分依据**：如"格力-核心部件L5→10分"、"美的-售后-地市级覆盖→7分"

### 评分卡表格模板

| 品牌 | 核心部件(30%) | 品控(25%) | 售后(20%) | 技术纯粹(15%) | 价格工程比(10%) | **总分** | 梯队 |
|------|:------------:|:---------:|:---------:|:------------:|:--------------:|:--------:|:----:|
| A | 10 | 9 | 8 | 9 | 7 | **8.95** | 第一梯队 |
