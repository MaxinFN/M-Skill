...
每次分析必须包含以下全部内容（按 14 章节固定标题和编号输出，不可跳过、不可改名）：
...
## 4. 制造深度分级
## 5. 技术路线对比结论
## 6. 可靠性分析（Bathtub + FMEA + 高风险部件）
## 7. 用户体验维度权重
## 8. 营销功能过滤列表（A/B/C 三级）
## 9. TCO判断（≥3 档机型对比 + 一句话结论）
## 10. 供应链与产业结构
## 11. 安装/环境影响（便携工具类改名为"使用与维护影响"）
## 12. 品牌工程评级（5 维评分卡）
## 13. 购买决策系统（结构化 Step 1–6 流程）
## 14. 一句话结论
...
> **强约束：一句话结论不是"总结"，是"工程判断"。** 必须包含以下三个要素，缺一不可。

### 三要素模板

```
[物理本质] — 该家电在物理学上到底是什么系统？1 句话。
[关键瓶颈] — 决定这台设备长期价值的最大工程约束是什么（不是参数，是物理/制造/安装层面的硬约束）？1 句话。
[工程甜点] — 在这个品类的技术和市场中，哪一档配置/价格代表了"工程价值/价格比"的最高点？1 句话。
```
...
| 禁止写法 | 为什么禁止 |
|----------|-----------|
| "综上所述，XX 是一个很好的选择" | 模糊评价，没有工程信息 |
| "推荐购买 XX 品牌，性价比高" | 品牌优先——结论不应是品牌推荐 |
| "XX 技术是未来的发展方向" | 推测未来而非判断当下 |
| 只写物理本质但不写工程甜点 | 缺少购买决策的工程锚点 |
| 只写推荐但不写物理本质 | 推荐缺少第一性原理支撑 |

### 正例（参考）

> 冰箱的工程本质是"逆卡诺循环驱动的稳态热管理系统"——压缩机是心脏，保温层和门封决定了心脏需要工作多少时间。冰箱最大的工程约束是 **24小时×365天连续运行的可靠性**——这是它区别于所有间歇使用家电的根本差异，也是"风冷双循环"对"直冷单循环"的碾压不在功能而在防腐和温度均匀性的原因。在这个品类中，**¥3000–4000 的风冷双循环变频冰箱**代表了工程价值/价格的最高点——你为双循环和变频付出的溢价会在 3–5 年内通过电费和食物保鲜回收，而超过 ¥6000 的增量支出主要买的是面板材质和内饰而非制冷工程。

### 输出格式

一句话结论在 HTML 中使用 `.conclusion` 容器输出，1 个段落包含三要素，不加编号、不分点。段落长度控制在 3–6 句。
